...
func writePromptCommandAndFlags(cfg *writePromptConfig, cfgPath pflag.Value) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "write",
		Short: "Write prompt on standard output",
		Example: "\n\t" +
			app.Name() + "--left\n\t" +
			app.Name() + "--right --config ~/prompto.cfg",
		SilenceErrors:         true,
